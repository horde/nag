...
<?php if (in_array('tasklist', $this->columns)): ?>
  <th id="s<?php echo Nag::SORT_OWNER ?>" class="horde-split-left<?php if ($this->sortby == Nag::SORT_OWNER) {
      echo ' ' . $this->sortdirclass;
  } ?>" width="2%">
...
<?php endif;
  if (in_array('start', $this->columns)): ?>
  <th id="s<?php echo Nag::SORT_START ?>" class="horde-split-left<?php if ($this->sortby == Nag::SORT_START) {
      echo ' ' . $this->sortdirclass;
  } ?>" width="2%">
   <?php echo $this->headerWidget($this->baseurl, $this->sortdir, $this->sortby, Nag::SORT_START, _("_Start Date")) ?>
  </th>
<?php endif;
  if (in_array('estimate', $this->columns)): ?>
  <th id="s<?php echo Nag::SORT_ESTIMATE ?>" class="horde-split-left<?php if ($this->sortby == Nag::SORT_ESTIMATE) {
      echo ' ' . $this->sortdirclass;
  } ?>" width="10%">
   <?php echo $this->headerWidget($this->baseurl, $this->sortdir, $this->sortby, Nag::SORT_ESTIMATE, _("Estimated Time")) ?>
  </th>
<?php endif;
  if (in_array('assignee', $this->columns)): ?>
  <th id="s<?php echo Nag::SORT_ASSIGNEE ?>" class="horde-split-left<?php if ($this->sortby == Nag::SORT_ASSIGNEE) {
      echo ' ' . $this->sortdirclass;
  } ?>" width="10%">
