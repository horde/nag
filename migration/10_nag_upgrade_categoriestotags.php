...
class NagUpgradeCategoriesToTags extends Horde_Db_Migration_Base
{
    protected function _init()
    {
        ...
        $types = $type_mgr->ensureTypes(['task']);
        $this->_type_ids = ['task' => (int) $types[0]];
    ...
    {
        $this->_init();
        $this->addColumn('nag_tasks', 'task_category', 'string', ['limit' => 80]);
        ...
        foreach ($rows as $row) {
            ...
            $this->update($sql, [reset($tags[$row['task_uid']]), (string) $row['task_uid']]);
        }
